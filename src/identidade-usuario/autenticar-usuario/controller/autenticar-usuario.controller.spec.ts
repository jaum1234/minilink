...
describe("AutenticarUsuarioControllerTest", () => {
  ...
  let mockAutenticarUsuarioService = {
    compararSenhas: jest.fn(),
    gerarJwt: jest.fn(),
  }

  beforeEach(async () => {
    controller = new AutenticarUsuarioController(mockAutenticarUsuarioService as any, mockUsuarioService as any);
  ...
  it("Deve retornar 404 se o usuário não for encontrado", async () => {
    ...
    expect(mockUsuarioService.buscarPorEmail).toHaveBeenCalledWith("nonexistent@example.com");
  ...
  it("Deve retornar 400 se a senha estiver incorreta", async () => {
    ...
    expect(mockUsuarioService.buscarPorEmail).toHaveBeenCalledWith(dados.email);
    expect(mockAutenticarUsuarioService.compararSenhas).toHaveBeenCalledWith(dados.senha, mockUsuario.senha);
    expect(mockResponse.status).toHaveBeenCalledWith(400);
    expect(mockResponse.json).toHaveBeenCalledWith({ mensagem: "Senha incorreta." });
  });
});

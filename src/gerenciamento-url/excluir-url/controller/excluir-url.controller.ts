import { Controller, Delete, HttpStatus, Param, Request, Res, UseGuards } from "@nestjs/common";
...
export class ExcluirUrlController {
  ...
  async excluir(
    ...
    if (url.usuario.id !== usuario.id) {
      return res.status(HttpStatus.FORBIDDEN).json({ message: 'Você não tem permissão para excluir esta Url' });
    }

import { Body, Controller, HttpStatus, Param, Patch, Request, Res, UseGuards } from "@nestjs/common";
...
export class AtualizarUrlController {
  ...
  async atualizar(
    ...
    if (url === null) {
      return res.status(HttpStatus.NOT_FOUND).json({ message: "Url não encontrada" });
    }

    if (url.usuario.id !== usuario.id) {
      return res.status(HttpStatus.FORBIDDEN).json({ message: "Você não tem permissão para atualizar esta Url" });
    }

    await this.urlService.atualizar(Number.parseInt(urlId), atualizarUrlDto.origem);

    return res.status(HttpStatus.OK).json({ message: "Url atualizada com sucesso" });
  }
}